...
def parse_exam_question(html_content):
    ...
    question_details = soup.find("div", class_="question-discussion-header").get_text(
        " ", strip=True
    )

    # Extract question
    question_text = soup.find("div", class_="question-body").get_text(" ", strip=True)
    ...
    for li in soup.find_all("li", class_="multi-choice-item"):
        ...
        choice_text = (
            li.get_text(" ", strip=True).replace(choice_letter + ".", "", 1).strip()
        )
    ...
    # Format output
    output = (
        f"Exam question from {exam_title}\n{question_details}\n\n{question_text}\n\n"
    )
    ...
    output += f"\nSuggested Answer: {suggested_answer} 🗳️\n\n"
